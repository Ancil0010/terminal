...
std::wstring TextBuffer::DumpAsVT()
{
    const auto lastRowWithText = GetLastNonSpaceCharacter(nullptr).y;
    std::wstring out;
    ...
    memset(&previousFg, 0xff, sizeof(previousFg));
    memset(&previousBg, 0xff, sizeof(previousBg));
    memset(&previousUl, 0xff, sizeof(previousUl));

    for (til::CoordType y = 0; y <= lastRowWithText; ++y)
    {
        ...
        {
            const auto newX = x + run.length;
            const auto attr = run.value.GetCharacterAttributes();
            const auto hyperlinkId = run.value.GetHyperlinkId();
            const auto fg = run.value.GetForeground();
            const auto bg = run.value.GetBackground();
            const auto ul = run.value.GetUnderlineColor();
            ...
            if (previousAttr != attr)
            {
                ...
                {
                    switch (run.value.GetUnderlineStyle())
                    {
    ...
    if (GetRowByOffset(lastRowWithText).WasWrapForced())
    {
        out.append(L"\r\n");
    }
